...
const addUser = async (req,res)=>{
    try {
        ...
        const {name, email, phone, adminId} = userData;
        ...
        if(!phone){
            return res.status(400).send({status : false, message : "user phone is required"})
        }
        if(!adminId){
            return res.status(400).send({status : false, message : "user adminId is required"})
        }
        ...
        return res.status(500).send({status : false, message : "error in add user", error : error.message})
    }
}
